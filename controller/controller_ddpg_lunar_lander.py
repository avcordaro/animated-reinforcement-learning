...
class ControllerDDPGLunarLander(Controller):
    """
    The Controller class for running a Deep Deterministic Policy Gradient agent in the Lunar Lander environment.
    ...
    def read_config_file(self):
        ...
        if os.path.exists("config.txt"):
            ...
            self.agent.GAMMA = config["ddpg-lunar-lander"].getfloat("gamma")
            self.agent.TAU = config["ddpg-lunar-lander"].getfloat("tau")
            self.agent.ALPHA_ACTOR = config["ddpg-lunar-lander"].getfloat("alpha_actor")
            self.agent.ALPHA_CRITIC = config["ddpg-lunar-lander"].getfloat("alpha_critic")
            self.agent.BATCH_SIZE = config["ddpg-lunar-lander"].getint("batch_size")
            self.epsilon = config["ddpg-lunar-lander"].getfloat("epsilon")
            self.epsilon_decay = config["ddpg-lunar-lander"].getfloat("epsilon_decay_amount")
            self.FINAL_EPSILON = config["ddpg-lunar-lander"].getfloat("final_epsilon")
            self.no_of_episodes = config["ddpg-lunar-lander"].getint("episodes")
    ...
    def load_best_model(self, *args):
        ...
        if self.gui.load_best_model.get() == 1:
            if os.path.exists("neural_network_models/lunar_lander_best_parameters.pth"):
                self.loaded_best_model = True
                self.agent.actor.load_state_dict(torch.load("neural_network_models/lunar_lander_best_parameters.pth"))
            else:
                self.gui.load_best_model.set(0)
                messagebox.showinfo("No Model Found", "Cannot find file \'lunar_lander_best_parameters.pth\' "
